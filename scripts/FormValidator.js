...
export default class FormValidator {
  ...
  _hiddenError = (inputSelector, errorInput, options) => {  //ф-ция скрывает ошибку
    errorInput.textContent = '';
    errorInput.classList.remove(options.errorClassActive);
    inputSelector.classList.remove(options.inputErrorClass);
  };
  
  _showError = (inputSelector, errorInput, message, options) => {  //ф-ция показывает ошибку
  ...
  _toggleErrorState = (inputSelector, formSelector, options) => {   //проверка на валидность
  ...
  _disableButton = (submitButton, options) => { //кнопка не активна
    submitButton.setAttribute('disabled', 'true');
    submitButton.classList.add(options.inactiveButtonClass);
  };
  
  _enableButton = (submitButton, options) => { //кнопка активна
    submitButton.removeAttribute('disabled');
    submitButton.classList.remove(options.inactiveButtonClass);
  };
  _togglePopupAddButton = (inputs, submitButton, options) => {   //активна кнопка, если оба поля валидны
...
//вызов метода после создания класса
